...
### Frequency Tables
A frequency table is a simple table that lists a variable and its value or the number of times it has occurred, they are commonly used in tallying. Below is a frequency table of pets in a hypothetical town.
...
| ![Temperature Bar Chart Middle East-Turkey--1901-2020--2021-07-13.png](https://upload.wikimedia.org/wikipedia/commons/7/71/Temperature_Bar_Chart_Middle_East-Turkey--1901-2020--2021-07-13.png) |
| :--: |
...
| ![Table_of_gender_by_major_with_percents.png](https://upload.wikimedia.org/wikipedia/commons/2/29/Table_of_gender_by_major_with_percents.png) |
| :--: |
...
| ![Histogram_of_arrivals_per_minute.svg](https://upload.wikimedia.org/wikipedia/commons/c/c3/Histogram_of_arrivals_per_minute.svg) |
| :--: |
...
The histogram is one of the more popular graphics because of of its intuitivity, and it allows us to understand certain features about data such as skewness, in addition to approximations of mean, median, and the mode within a particular set of values.

### Scatter Plots

| ![Enwiki.damaging_goodfaith.proba_scatter.svg/2560px-Enwiki.damaging_goodfaith.proba_scatter.svg.png](https://upload.wikimedia.org/wikipedia/commons/thumb/e/ec/Enwiki.damaging_goodfaith.proba_scatter.svg/2560px-Enwiki.damaging_goodfaith.proba_scatter.svg.png) |
| :--: |
...
| ![2048px-Michelsonmorley-boxplot.svg.png](https://upload.wikimedia.org/wikipedia/commons/thumb/f/fa/Michelsonmorley-boxplot.svg/2048px-Michelsonmorley-boxplot.svg.png) |
| :--: |
| *Boxplot representing Michelson's data on the speed of light. It consists of five experiments, each made of 20 consecutive runs. Created using [R](/The-R-Language). User:Schutz, Public domain, via Wikimedia Commons* |

The box plot, or box-and-whisker diagram, is a graphic used in statistics and biostatistics to understand the distributions, locality, spread and skewness of data values for any varaible. The box plot divides the data into quartiles that can then be used to further analyze the data, it is useful in many applications including but not limited to mean/median/mode calculations, skewness estimation, interquartile range calculation, determining maximum and minimum, finding percentiles, and determining outliers.
...
| ![Normal_distribution_pdf.png](https://upload.wikimedia.org/wikipedia/commons/1/1b/Normal_distribution_pdf.png) |
| :--: |
...
The mean is the average of all the data values in the set, the median is the value that sits at the center of the data set once all the data is ordered, and the mode is the most commonly occuring value within the dataset. The formula for the mean is shown below:

$$ \large{\overline{x} = \frac{\Sigma^{n}_{i=1}x_i}{n}} $$

<h4> Finding The Median </h4>

| ![Finding_the_median.png](https://upload.wikimedia.org/wikipedia/commons/c/cf/Finding_the_median.png) |
| :--: |
| *How to find the median in both even or odd datasets. Blythwood, [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0), via Wikimedia Commons* |
...
| ![BimodalAnts.png](https://upload.wikimedia.org/wikipedia/commons/f/f2/BimodalAnts.png) |
| :--: |
...
| ![Comparison_standard_deviations.svg](https://upload.wikimedia.org/wikipedia/commons/f/f9/Comparison_standard_deviations.svg) |
| :--: |
...
| ![Tula_gingerbread.jpg](https://upload.wikimedia.org/wikipedia/commons/0/09/Tula_gingerbread.jpg) |
| :--: |
...
The shape of a frequency distribution or a histogram can either be centered (unimodal or multimodal), left-skewed, right-skewed, asymmetrical multimodal, or uniform. Identifying if a a frequency distribution is left-skewed or right-skewed can be counter-intuitive at times, for that reason it is helpful to imagine a uniform distribution without any skew; a single large slab like the dessert above, if you are to take a bite from the left half of the dessert, it will resemble a left-skewed distribution, if you fancy a bite from the right half then it will resemble a right-skewed distribution.

## Percentiles

Elaborating more on the previous section ["Box plots"](#box-plots), box plots help in dividing the data into four quartiles. The first quartiles covers the data points that fall in the 0th-25th percentile, the second quartile covers the data point within the 25th-50th percentile, and likewise the third quartile covers the 50th-75th percentile, and the 4th percentile shows us the data points within the 75th-100th percentiles.

The uses of ordering data like this is that we can determine a few key properties of the dataset, the first being the range which is equal to the difference between the largest and smallest data points which lie on the ends of a box plot chart:

$$ Range=Maximum-Minimum $$

Another imporant metric that can be determined using quartiles when data is ordered is the interquartile range or IQR, to determine the IQR it is the differecne between the third and first quartiles.
...
Naturally the true definition of outlier is debated as there is no concrete way of really undertanding what is or isn't an outlier in within any particular dataset, however this level of understanding is sufficient for more foundational statistical studies, however the [Further readings](#references--further-readings) will contain additional information for those who are interested[^4].
